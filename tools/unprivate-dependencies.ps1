...
foreach ($file in $filesToPublicize) {
    ...
    if ($IsLinux) {
        $publicizerExe = Join-Path $cwd "../tools/AssemblyPublicizer/AssemblyPublicizer.exe"
        Start-Process -FilePath "/usr/bin/mono" -WorkingDirectory $rawDepsPath -ArgumentList @($publicizerExe, "-i", $filePath, "-o", $fileDest) -Wait
    } else {
        Start-Process -FilePath "../tools/AssemblyPublicizer/AssemblyPublicizer.exe" -WorkingDirectory $rawDepsPath -ArgumentList @("-i", $filePath, "-o", $fileDest) -Wait
    }
}
